...
bool FontSDL::load(const std::string& file, size_t size)
{
    this->size = size;
    ...
    Uint32 w = this->atlasSize.x;
    Uint32 h = this->atlasSize.y;
    ...
    this->textureData = new Uint32[w*h];
    ...
    this->texture = std::make_shared<TextureSDL>();

    if (!this->texture->load(reinterpret_cast<unsigned char*>(this->textureData), w, h))
    {
        knM_logError("Can't load texture from font atlas: '" << file << "'");
        return false;
    }
...
Font::Glyph FontSDL::createGlyphInfo(int codepoint) const
{
    Glyph g;
...
const Font::Glyph* FontSDL::getGlyphInfo(int codepoint) const
{
...
bool FontSDL::loadTexture(const RendererSDL& renderer)
{
    return this->texture->loadTexture(renderer);
}
